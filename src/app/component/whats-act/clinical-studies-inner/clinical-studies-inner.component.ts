...
export class ClinicalStudiesInnerComponent implements OnInit {
  pageTitle = "";
  headerSection: any;
  footerSection: any;
  studyDetails;
      ...
      if((environment.mainDomain.includes(window.location.origin) && item?.data?.en_uid?.toLowerCase() == studyName.toLowerCase())
        ...
        if (obj != null && obj.indexOf('body') > -1 && !Number.isNaN(obj.replace("body", ""))) {
          ...
          switch (prismic.slice_type) {
            ...
            case 'clinical_studies_details':  
            prismic.primary.studies_name = clinicalStudiesData.clinical_studies_name[0].text;
